  ...
  const handleClick = (event) => {
    ...
    if (searchTerms.what.trim() && searchTerms.where.trim()) {
      ...
      let newJobList = jobList.filter((job) => {
        return (
          job.title.toLowerCase().includes(searchTerms.what) ||
          job.company.toLowerCase().includes(searchTerms.what)
        );
      });
      setJobList(
        newJobList.filter((job) =>
          job.location.toLowerCase().includes(searchTerms.where)
        )
      );
    } else if (searchTerms.what.trim() && !searchTerms.where.trim()) {
      //What exists
    ...
    <div className="App">
      ...
      <Switch>
        <Route
          path="/"
          exact
          render={(routerProps) => <LandingPage handleClick={handleClick} />}
        />
        <Route
          path="/jobs"
          exact
          render={() => (
            <ResultsPage
              ...
              handleClick={handleClickJobListing}
            />
          )}
        />
